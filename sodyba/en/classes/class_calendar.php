...
public $booking_start_time          = "18:00";			// The time of the first slot in 24 hour H:M format  
public $booking_end_time            = "19:00"; 			// The time of the last slot in 24 hour H:M format  
public $booking_frequency           = 1200;   			// The slot frequency per hour, expressed in minutes.  	
...
function make_cells($table = '') {
	...
	foreach($this->days as $i => $r) { // Loop through the date array
		...
		if(isset($this->bookings_per_day[$current_day]) && $tag == 0) {
			...
			$current_day_slots_booked = count($this->bookings_per_day[$current_day]);
				...
				if($current_day_slots_booked < $this->slots_per_day) {
					...
					<a href='book.php?month=" .  $this->month . "&amp;year=" .  $this->year . "&amp;day=" . sprintf("%02s", $r['daynumber']) . "' class='part_booked' title='This day is part booked'>" . 
					$r['daynumber'] . "</a></td>"; 
					$tag = 1;
					...
					echo "\r\n<td width='21' valign='top'>
					<a href='book.php?month=" .  $this->month . "&amp;year=" .  $this->year . "&amp;day=" . sprintf("%02s", $r['daynumber']) . "' class='fully_booked' title='This day is unavailable'>" . 
					$r['daynumber'] . "</a></td>"; 
					$tag = 1;			
		...
		if($tag == 0) {
			...
			echo "\r\n<td width='21' valign='top'>
			<a href='book.php?month=" .  $this->month . "&amp;year=" .  $this->year . "&amp;day=" . sprintf("%02s", $r['daynumber']) . "' class='green' title='Užsakymų peržiūra'>" . 
			$r['daynumber'] . "</a></td>";			
...
function booking_form() {
	...
	echo "
	<div id='outer_booking'><h2>Choose homestead type</h2>

	<p>
	This date is available: <span> " . $this->year . "-" . $this->month . "-" . $this->day . "</span>
	</p>
	
	<table width='400' border='0' cellpadding='2' cellspacing='' id='booking'>
		<tr>
			<th width='150' align='left'>Homestead type</th>
		...
		$result = $this->link->query("SELECT * FROM types");
		...
		foreach($types as $type) {			
			...
			<tr>
				<td>" . $type[1] . "</td>
				<td>" . $this->cost_currency_tag . number_format($type[2], 2) . "</td>
				<td width='110'><input data-date='" . $slots[0] . "' data-price='".number_format($type[2], 2)."' data-type='" . $type[1] . "' class='fields' type='checkbox'/></td>
			</tr>";
		...
		<div id='selected_slots'></div>		
			...
			<div id='basket_details'>
				...
				<form method='post' action='book_slots.php'>
					...
					<input type='submit' class='classname' value='Book'>
